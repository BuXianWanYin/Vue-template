...
const props = defineProps<Props & { currentUser: { userId: string | number, userName: string } }>()
...
/** 查询饵料信息列表 */
const getBaitInfoList = async () => {
  const response = await AgricultureMaterialService.listMaterial(queryParams)
  // 只保留 status 为 1 的数据
  baitInfoList.value = response.rows
