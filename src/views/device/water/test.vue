<template>
    <div class="water-quality">
        ...
        <el-card class="box-card">
          <template #header>
            <div class="card-header">
              <span>水质异常预警</span>
              <div class="header-right">
                <el-select v-model="warningLevel" size="small" placeholder="报警级别">
          ...
          <el-table :data="filteredWarningList" v-loading="warningLoading" style="width: 100%">
      ...
      <!-- 编辑报警规则弹窗 -->
      <el-dialog v-model="editDialogVisible" title="编辑报警规则" width="500px">
  ...
  const fetchWarningList = async () => {
    ...
    try {
      const res = await AgricultureDeviceSensorAlertService.listAlert({
  ...
  const monitorParams = computed(() => [
  {
    name: 'do',
    label: '溶解氧(DO)',
    value: waterQualityTrendData.value?.do ?? '--',
    unit: 'mg/L',
    range: '5-8 mg/L',
    percentage: 85, // 可根据实际算法替换
    status: { type: 'success', text: '正常', color: '#67C23A' }, // 可根据实际算法替换
    trend: waterQualityTrendData.value?.trend?.do ?? { type: 'info', text: '--' }
  },
  {
    name: 'ph',
    label: 'pH值',
    value: waterQualityTrendData.value?.ph ?? '--',
    unit: '',
    range: '6.0-7.0',
    percentage: 90,
    status: { type: 'warning', text: '偏高', color: '#E6A23C' },
    trend: waterQualityTrendData.value?.trend?.ph ?? { type: 'info', text: '--' }
  },
  {
    name: 'temperature',
    label: '水温',
    value: waterQualityTrendData.value?.temperature ?? '--',
    unit: '℃',
    range: '18-30 ℃',
    percentage: 70,
    status: { type: 'success', text: '正常', color: '#67C23A' },
    trend: waterQualityTrendData.value?.trend?.temperature ?? { type: 'info', text: '--' }
  },
  {
    name: 'ammonia',
    label: '氨氮',
    value: waterQualityTrendData.value?.ammonia ?? '--',
    unit: 'mg/L',
    range: '< 1.0 mg/L',
    percentage: 95,
    status: { type: 'danger', text: '超标', color: '#F56C6C' },
    trend: waterQualityTrendData.value?.trend?.ammonia ?? { type: 'info', text: '--' }
  }
  ]);
   ...
   watch(
    ...
    async (
      ...
      if (newPastureId !== oldPastureId || newBatchId !== oldBatchId) {
        ...
        console.log('大棚id:', newPastureId)
        console.log('分区id:', newBatchId)
  ...
  const onHandleSave = async () => {
    try {
      // 假设有API: AgricultureDeviceSensorAlertService.updateAlert
  ...
  const filteredWarningList = computed(() => {
    return warningList.value.filter(item => {
      ...
      if (warningLevel.value) {
        // alertLevel: 0(严重) danger, 1(警告) warning
        if (warningLevel.value === 'danger') {
          levelOk = item.alertLevel === 0
        } else if (warningLevel.value === 'warning') {
          levelOk = item.alertLevel === 1
        }
      }
      // 状态筛选
      let statusOk = true
      if (warningStatus.value !== '') {
        statusOk = item.status === warningStatus.value
      }
  ...
  const markAllAsHandled = async () => {
    // 找出当前筛选下未处理的预警
    const toHandle = filteredWarningList.value.filter(item => item.status === 0)
  ...
  const paramMap = {
    do: { name: '溶解氧', data: data.do, unit: 'mg/L' },
    ph: { name: 'pH值', data: data.ph, unit: '' },
    temperature: { name: '水温', data: data.temperature, unit: '℃' },
    ammonia: { name: '氨氮', data: data.ammonia, unit: 'mg/L' }
  };
  ...
  const option = {
    grid: { left: '2%', right: '2%', top: 60, bottom: 40, containLabel: true },
    tooltip: {
      trigger: 'axis',
  ...
  const getTrend = (paramName) => {
    let trendMsg =  waterQualityTrendData.value
    if (
      !trendMsg ||
      !trendMsg.deviceInfo ||
    ...
    if (
      ...
      let type = 'info'
      if (trendMsg.data[paramName].trend === '上升') type = 'success'
