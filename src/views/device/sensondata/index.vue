...
const handleCascaderChange = async (val) => {
  ...
  // 查询该分区下的设备
  const res = await AgricultureDeviceService.listDevice({ pastureId, batchId, pageNum: 1, pageSize: 9999 })
  if (res.code === 200) {
...
onMounted(async () => {
  ...
  const res = await AgricultureDeviceService.listDevice({ pageNum: 1, pageSize: 9999 })
  if (res.code === 200) {
    // 统计每个大棚-分区下的设备数量
    const countMap = {}
